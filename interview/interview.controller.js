...
class interviewController {
  getAnswers = AsyncWrapper(async (req, res, next) => {
    const answers = await interviewService.getAnswer(
      req.body.interviewId,
      req.body.qId,
      req.body.answer
    ); // get arrays of question's ids and answers
  ...
  getInterviewInfo = AsyncWrapper(async (req, res, next) => {
    ...
    const interview = await interviewService.getInterviewInfo(
      req.user.id,
      interviewId
    );
  ...
  getInterviewDetails = AsyncWrapper(async (req, res, next) => {
    const { interviewId } = req.params; // Correct param retrieval
    const interviewDetails = await interviewService.getInterviewDetails(
      interviewId
    );
    //question?role=frontend
